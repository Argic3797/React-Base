...
const getAverage = (numbers:number[]) => {   //Average라는 함수를 조회하는 함수이다
...
function Memo() {
    ...
    const onInsert = (event: React.MouseEvent<HTMLButtonElement>) => {
        // concat: Array 인스턴스의 concat() 메서드는 두 개 이상의 배열을 병합하는 데 사용됩니다.
        // paseInt: parseInt() 함수는 문자열 인자를 파싱하여 특정 진수(수의 진법 체계에서 기준이 되는 값)의 정수를 반환합니다.
        // 이 메서드는 기존 배열을 변경하지 안고, 새 배열을 반환합니다.
